...
class EncoderRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        h = torch.zeros((self.num_layer, batch_size,
                         self.hidden_size))  # torch.empty((self.num_layer, batch_size, self.hidden_size)).normal_(mean=0.0, std=0.01)
        c = torch.zeros((self.num_layer, batch_size,
                         self.hidden_size))  # torch.empty((self.num_layer, batch_size, self.hidden_size)).normal_(mean=0.0, std=0.01)
        h = h.cuda()
        c = c.cuda()
        ...
        outputs = torch.zeros((self.max_seq_length, self.hidden_size))

        for time_step in range(seq_len):
            input = self.embedding(x[time_step, 0, 0]).view(1, 1,
                                                            -1)  # this view is necessary to transform shape [256] -> [1,1,256]
            input = F.relu(input)
...
class DecoderAttention(nn.Module):
    def __init__(self, hidden_size, embedding_size, y_vocabulary_size, max_seq_length, loss_criterion, target_SOS_idx,
                 target_EOS_idx):
        ...
        # Attention part
        self.attention = nn.Linear(self.hidden_size + self.embedding_size, self.max_seq_length)
        torch.nn.init.xavier_normal_(self.attention.weight)
        self.attention.bias.data.zero_()
        self.attention_combiner = nn.Linear(self.hidden_size + self.embedding_size, self.hidden_size)
        torch.nn.init.xavier_normal_(self.attention_combiner.weight)
        self.attention_combiner.bias.data.zero_()
    ...
    def forward(self, last_hidden, encoder_outputs, y=None, mode='train'):
        ...
        h = last_hidden.view(self.num_layer, 1, self.hidden_size)  # this is now given by the encoder!
        c = torch.zeros((self.num_layer, 1,
                         self.hidden_size))  # torch.empty((self.num_layer, batch_size, self.hidden_size)).normal_(mean=0.0, std=0.01)
        h = h.cuda()
        c = c.cuda()
        ...
        if mode == 'train':
            ...
            for time_step in range(seq_len - 1):
                embedded_word_input = self.embedding(y[time_step, 0, 0]).view(1, 1, -1)
                embedded_word_input = F.relu(embedded_word_input)

                # Attention mechanism
                # squeeze e unsqueeze, h is shape [1,1,hidden], embedded is [1, 1, embedded] I want to
                # concatenate it into a single vector with [1, hidden + embedded] so I can feed it into
                # the fully connected, that is the attention
                attention_input = torch.cat((h.squeeze(0), embedded_word_input.squeeze(0)), dim=1) # [1, hidden + embbed ]
                attention_output = self.attention(attention_input) # [1, max_seq_len]
                attention_weights = self.softmax(attention_output)    # [1, max_seq_len]
                # This operation applies unsqueeze to attention weights so it becomes [1,1,max_seq_len],
                # while the encoder outputs are shaped as [ max_seq_len, hidden_size ] we unsqueeze it into
                # [ 1, max_seq_len, hidden_size ] so when we do batch matrix multiplication is just as doing a VECTOR
                # X MATRIX MULTIPLICATION, the result is therefore [ 1, 1, hidden_size ]
                #print(encoder_outputs.shape)
                #print(attention_weights.shape)
                # The result is a single hidden vector, obtained by multiplying
                #      attention coefficient 1 to the whole encoder output vector produced at time step 1
                #      attention coefficient 2 to the whole encoder output vector produced at time step 2
                #       ....
                #      attention coefficient Max_Seq_Len to the whole encoder output vector produced at time step Max_Seq_Len
                # And FINALLY SUM EVERY VECTOR
                # So at the end, what SURVIVES as information is the one which attention coefficient was HIGHEST
                #
                # ALSO NOTE THAT: The Padding is "automatic", since encoder_outputs are initialized as ZERO for the
                # time step below max_Seq_len, these hidden vector are never going to influence the final result
                attentive_hidden_vector = torch.bmm(attention_weights.unsqueeze((0)), encoder_outputs.unsqueeze(0))
                h = self.attention_combiner(torch.cat((attentive_hidden_vector.squeeze(0), embedded_word_input.squeeze(0)), dim=1)) # [ 1, hidden_size ]
                h = h.unsqueeze(0) # [ 1, 1, hidden_size ] -> this is necessary for the LSTM
                # back as normal

                out, (h, c) = self.lstm(embedded_word_input, (h, c))
                # lascio h or c?
            ...
            for time_step in range(self.max_seq_length):
                embedded_word_input = self.embedding(input).view(1, 1, -1)
                embedded_word_input = F.relu(embedded_word_input)

                # Attention mechanism
                attention_input = torch.cat((h.squeeze(0), embedded_word_input.squeeze(0)), dim=1)
                attention_output = self.attention(attention_input)
                attention_weights = self.softmax(attention_output)
                attentive_hidden_vector = torch.bmm(attention_weights.unsqueeze((0)), encoder_outputs.unsqueeze(0))
                h = self.attention_combiner(torch.cat((attentive_hidden_vector.squeeze(0), embedded_word_input.squeeze(0)), dim=1))
                h = h.unsqueeze(0)
                # back as normal
                ...
                print("vediamo il valore in topi: " + str(out[0][topi.item()]) + ' ' + str(
                    out[0][topi.item() + 1]) + ' ' + str(out[0][topi.item() - 1]))
...
if __name__ == "__main__":
    ...
    encoder.cuda()
    decoder = DecoderAttention(hidden_size, embedding_size, len(y_idx2word_list), MAX_SEQUENCE_LENGTH, loss_criterion,
                         y_word2idx_dict['SOS'], y_word2idx_dict['EOS'])
    decoder.cuda()
    ...
    # Load old state dict
    #encoder.load_state_dict(torch.load('./attention_encoder_lr' + str(learning_rate) + '.pth'))
    #decoder.load_state_dict(torch.load('./attention_decoder_lr' + str(learning_rate) + '.pth'))
    ...
    for epoch in range(num_epoch):
        ...
        for train_idx in range(num_train_sentences):
            ...
            input_tensor_x = torch.tensor(input_vector_x).view(-1,
            ...
            encoder_outputs, (last_hidden, _) = encoder(input_tensor_x)

            total_loss, output_words = decoder(last_hidden, encoder_outputs, y=target_tensor_y)
            # since we need to take into account also the length of the targets we need to divide the total loss
            # with the length of the target sequence,
            ...
            if iter % num_runs == 0:
                ...
                print(str(iter / (num_epoch * num_train_sentences) * 100) + " % it: " + str(iter) + " avg loss: " + str(
                    avg_loss))
                running_loss = 0
                ...
                if write_summary:
                    ...
                    writer.add_histogram('train_hist_decoder/decoder_embedding_weights',
                                         decoder._modules['embedding'].weight, iter)
                    writer.add_histogram('train_hist_decoder/decoder_out_fc_weights', decoder._modules['out_fc'].weight,
                                         iter)
                    writer.add_histogram('train_hist_decoder/decoder_out_fc_bias', decoder._modules['out_fc'].bias,
                                         iter)
                    writer.add_histogram('train_hist_decoder/decoder_lstm_weights_hh_l0',
                                         decoder._modules['lstm'].weight_hh_l0, iter)
                    writer.add_histogram('train_hist_decoder/decoder_lstm_bias_hh_l0',
                                         decoder._modules['lstm'].bias_hh_l0, iter)
                    writer.add_histogram('train_hist_decoder/decoder_lstm_weights_ih_l0',
                                         decoder._modules['lstm'].weight_ih_l0, iter)
                    writer.add_histogram('train_hist_decoder/decoder_lstm_bias_ih_l0',
    ...
    if save_to_file:
        torch.save(encoder.state_dict(), './attention_encoder_lr' + str(learning_rate) + '.pth')
        torch.save(decoder.state_dict(), './attention_decoder_lr' + str(learning_rate) + '.pth')
    if write_summary:
        writer.close()
    ...
    for pair_idx in range(len(test_x)):
        ...
        print("Input: " + str(test_x[pair_idx]) + " Pred " + \
              str(utils.convert_vector_idx2word(output_words, y_idx2word_list)) + " Gt: " + str(test_y[pair_idx]))
        exit()
